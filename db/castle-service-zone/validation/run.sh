#!/usr/bin/env bash
# Validate the castle service-zone migrations against a throwaway local database.
# This never touches the THYLORA backend. It answers: do these files apply cleanly
# and idempotently, and do the rules actually reject what they claim to reject?
#
#   sudo service postgresql start
#   db/castle-service-zone/validation/run.sh
set -uo pipefail
DB="${CSZ_VALIDATION_DB:-csz_check}"
...
cat "$HERE/behaviour.sql" | psql -q -d "$DB" 2>&1 | grep -E '^(==|ERROR|[a-z_]+ *[=|]|readback|NOTICE)'
exit $status
