#!/usr/bin/env bash
# Validate the world-castle migrations against a throwaway local PostgreSQL
# database. This never touches the THYLORA backend — and in the authoring
# environment it could not: egress to jvsdxhrfhtlgaknhjxlz.supabase.co is denied
# by network policy. It answers one question: do these migrations apply cleanly,
# idempotently, and do the rules actually reject what they claim to reject?
#
#   sudo service postgresql start
#   db/world-castle/validation/run.sh
set -uo pipefail
DB="${WORLD_CASTLE_VALIDATION_DB:-worldcastle_check}"
