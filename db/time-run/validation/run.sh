#!/usr/bin/env bash
# Validate the Time Run migrations against a throwaway local PostgreSQL database.
# This never touches the THYLORA backend. It answers one question: do these
# migrations apply cleanly, idempotently, and do the locks actually reject what
# they claim to reject?
#
#   sudo service postgresql start
#   db/time-run/validation/run.sh
set -uo pipefail
DB="${TIMERUN_VALIDATION_DB:-timerun_check}"
...
echo "== resetting $DB"
psql -q -d "${PGADMIN_DB:-postgres}" -c "drop database if exists $DB" >/dev/null 2>&1
psql -q -d "${PGADMIN_DB:-postgres}" -c "create database $DB"       >/dev/null 2>&1
