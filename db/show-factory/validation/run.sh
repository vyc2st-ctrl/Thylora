#!/usr/bin/env bash
# Validate the History → Show Factory migrations against a throwaway local database.
...
set -uo pipefail
DB="${HSF_VALIDATION_DB:-hsf_check}"
...
echo "== resetting $DB"
psql -q -d "${HSF_ADMIN_DB:-postgres}" -c "drop database if exists $DB" >/dev/null 2>&1
psql -q -d "${HSF_ADMIN_DB:-postgres}" -c "create database $DB" >/dev/null 2>&1
