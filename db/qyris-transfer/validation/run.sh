#!/usr/bin/env bash
# Validate the QYRIS TRANSFER migrations against a throwaway local PostgreSQL database.
# This never touches the THYLORA backend. It answers three questions:
#   1. do these migrations apply cleanly and idempotently?
#   2. does the recursion refuse to drop a frontier or transfer to nowhere?
#   3. does a transfer actually create the next context?
#
#   sudo service postgresql start
#   db/qyris-transfer/validation/run.sh
set -uo pipefail
DB="${QYRIS_TRANSFER_VALIDATION_DB:-qyris_transfer_check}"
